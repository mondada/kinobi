...
class Mysqldump
{
    ...
    public function __construct(
        ...
        $dumpSettingsDefault = array(
            'include-tables' => array(), // Only include these tables (array of table names), include all if empty
            'exclude-tables' => array(), // Exclude these tables (array of table names), include all if empty, supports regexps
            'compress' => Mysqldump::NONE, // Gzip, Bzip2, None
            'init_commands' => array(),
            'no-data' => array(), /* Do not dump table contents */
            'reset-auto-increment' => false, // Removes the AUTO_INCREMENT option from the database definition
            ...
            'add-drop-trigger' => true, /* Add DROP TRIGGER statement before each CREATE TRIGGER statement */
            'add-locks' => true, /* Surround each table dump with LOCK TABLES and UNLOCK TABLES statements */
            'complete-insert' => false, /* Use complete INSERT statements that include column names */
            'databases' => false, /* Interpret all name arguments as database names */
            'default-character-set' => Mysqldump::UTF8, /* Specify default character set */
            'disable-keys' => true, /* For each table, surround INSERT statements with statements to disable and enable keys */
            'extended-insert' => true, /* Use multiple-row INSERT syntax */
            'events' => false, /* Dump events from dumped databases */
            'hex-blob' => true, /* Dump binary columns using hexadecimal notation */
            'insert-ignore' => false, /* Write INSERT IGNORE rather than INSERT statements */
            'net_buffer_length' => self::MAXLINESIZE, /* Buffer size for TCP/IP and socket communication */
            'no-autocommit' => true, /* Enclose the INSERT statements for each dumped table within SET autocommit = 0 and COMMIT statements */
            'no-create-info' => false, /* Do not write CREATE TABLE statements that re-create each dumped table */
            'lock-tables' => true, /* Lock all tables before dumping them */
            'routines' => false, /* Dump stored routines (procedures and functions) from dumped databases */
            'single-transaction' => true, /* Issue a BEGIN SQL statement before dumping data from server */
            ...
            'skip-dump-date' => false, /* Exclude dump date as "Dump completed on" comment */
            'skip-definer' => false, /* Omit DEFINER and SQL SECURITY clauses from view and stored program CREATE statements */
        ...
        if ("sqlite" === $this->dbType) {
            $this->dumpSettings['single-transaction'] = false;
            $this->dumpSettings['exclude-tables'][] = "sqlite_sequence";
        }

        if ($this->dumpSettings['sqlite-dump']) {
            $this->dumpSettings['disable-keys'] = false;
            $this->dumpSettings['skip-comments'] = true;
            $this->dumpSettings['add-locks'] = false;
            $this->dumpSettings['extended-insert'] = false;
            $this->dumpSettings['reset-auto-increment'] = true;
        }
     ...
     *   sqlite:/opt/databases/testdb.sqlite
    ...
    private function parseDsn($dsn)
    {
        ...
        if ("sqlite" === $this->dbType) {
            if (empty($dsn)) {
                throw new Exception("Missing database path from DSN string");
            }

            if (!is_file($dsn)) {
                throw new Exception("Database from DSN string does not exist");
            }
    ...
    public function start($filename = '')
    {
        ...
        // Restore 'sqlite_sequence' values
        if ($this->dumpSettings['sqlite-dump']) {
            $this->compressManager->write(
                $this->getSQLiteSequence()
            );
        }
    ...
    private function getDumpFileHeader()
    {
        ...
        if ($this->dumpSettings['sqlite-dump']) {
            $header = "PRAGMA foreign_keys=OFF;".PHP_EOL.
                    "BEGIN TRANSACTION;".PHP_EOL;
        }
        return $header;
    }
    ...
    /**
     * Fills $this->autoIncrement array so it can be used later.
     *
     * @return null
     */
    private function getAutoIncrementValues()
    {
        foreach ($this->tables as $table) {
            if (empty($this->dumpSettings['include-tables'])) {
                foreach ($this->dbHandler->query($this->typeAdapter->show_auto_increment($this->dbName, $table)) as $row) {
                    $this->autoIncrement[$table] = current($row);
                }
            }
        }
    ...
    /**
     * Creates the staement to restore the sqlite_sequence values.
     *
     * @return string
     */
    private function getSQLiteSequence()
    {
        $sequence = '';

        if (!empty($this->autoIncrement)) {
            $sequence = "DELETE FROM sqlite_sequence;".PHP_EOL;
            foreach ($this->autoIncrement as $name => $seq) {
                $sequence .= "INSERT INTO sqlite_sequence VALUES('".$name."',".$seq.");".PHP_EOL;
            }
        }
    ...
    private function escape($colValue, $colType)
    {
        ...
        if ("sqlite" === $this->dbType) {
            ...
            $colValue = str_replace("\n", "\\n", str_replace("\r", "\\r", $colValue));
            return "'".$colValue."'";
        }

        return $colValue = $this->dbHandler->quote($colValue);
    }
    ...
    private function listValues($tableName)
    {
        ...
        foreach ($resultSet as $row) {
            ...
            if ($onlyOnce || !$this->dumpSettings['extended-insert']) {
                ...
                } elseif ($this->dumpSettings['sqlite-dump']) {
                    $lineSize += $this->compressManager->write(
...
class TypeAdapterSqlite extends TypeAdapterFactory
{
    ...
    {
        return "SELECT name as 'Trigger', sql as 'SQL Original Statement' ".
            "FROM sqlite_master ".
            "WHERE type='trigger' AND name='$triggerName'";
    }

    /**
     * function show_create_procedure Get procedure creation code from database
     * @todo make it do something with sqlite
     */
    ...
    /**
     * function show_create_event Get event creation code from database
     * @todo make it do something with sqlite
     */
    ...
    public function create_table($row)
    {
        ...
        if ($this->dumpSettings['sqlite-dump']) {
        	...
        	$createTable .= " ENGINE=".$this->dumpSettings['mysql-engine'];
        	if (!$this->dumpSettings['reset-auto-increment']) {
				$resultSet = $this->dbHandler->query("SELECT seq FROM sqlite_sequence WHERE name='".$row['Table']."';");
				$autoInc = $resultSet->fetchColumn();
				$resultSet->closeCursor();
				if (!empty($autoInc)) {
	        		$createTable .= " AUTO_INCREMENT=".$autoInc;
	        	}
        	}
        	$createTable .= " DEFAULT CHARSET=".$this->dumpSettings['default-character-set'];
            $ret = "/*!40101 SET @saved_cs_client     = @@character_set_client */;".PHP_EOL.
                "/*!40101 SET character_set_client = ".$this->dumpSettings['default-character-set']." */;".PHP_EOL.
                str_replace("AUTOINCREMENT", "AUTO_INCREMENT", $createTable).";".PHP_EOL.
    ...
    /**
     * function create_event Modify event code, add delimiters, etc
     * @todo make it do something with sqlite
     */
     ...
     *
     * @param array $colType Array returned from "pragma_table_info('tableName')"
     * @return array
     */
    public function parseColumnType($colType)
    {
        ...
        $colInfo['is_numeric'] = in_array($colInfo['type'], $this->sqliteTypes['numerical']);
        $colInfo['is_blob'] = in_array($colInfo['type'], $this->sqliteTypes['blob']);
    ...
    public function show_auto_increment()
    {
        $this->check_parameters(func_num_args(), $expected_num_args = 2, __METHOD__);
        $args = func_get_args();
        return "SELECT seq FROM sqlite_sequence WHERE name='${args[1]}'";
    }
...
class TypeAdapterMysql extends TypeAdapterFactory
{
    ...
    public function create_table($row)
    {
        ...
        if ($this->dumpSettings['sqlite-dump']) {
            // Kinobi code
            $match = "/ ENGINE=[a-zA-Z]+/s";
            $replace = "";
            $createTable = preg_replace($match, $replace, $createTable);
            $match = "/ DEFAULT CHARSET=[a-zA-Z0-9]+/s";
            $replace = "";
            $createTable = preg_replace($match, $replace, $createTable);
            $match = "/ COLLATE[ =][a-zA-Z0-9_]+/s";
            $replace = "";
            $createTable = preg_replace($match, $replace, $createTable);
            $match = "/CONSTRAINT.*FOREIGN KEY/s";
            $replace = "FOREIGN KEY";
            $createTable = preg_replace($match, $replace, $createTable);
            $match = "/[\s]+PRIMARY KEY [a-zA-Z0-9_` \(\),]+/s";
            $replace = "";
            $createTable = preg_replace($match, $replace, $createTable);
            $match = "/[\s]+KEY[a-zA-Z0-9_` \(\)]+,/s";
            $replace = "";
            $createTable = preg_replace($match, $replace, $createTable);
            $createTable = str_replace("int(11) NOT NULL AUTO_INCREMENT", "integer PRIMARY KEY AUTOINCREMENT NOT NULL", $createTable);
            $createTable = str_replace(" DEFAULT ''", "", $createTable);
            $createTable = str_replace(" ON UPDATE NO ACTION", "", $createTable);
            $createTable = str_replace("`", "", $createTable);
            $createTable = str_replace("'", "", $createTable);
            $createTable = str_replace(",".PHP_EOL.")", PHP_EOL.")", $createTable);
            // End Kinobi code
            $ret =  $createTable.";".PHP_EOL;
        } else {
            $ret = "/*!40101 SET @saved_cs_client     = @@character_set_client */;".PHP_EOL.
    ...
    public function show_auto_increment()
    {
        $this->check_parameters(func_num_args(), $expected_num_args = 2, __METHOD__);
        $args = func_get_args();
        return "SELECT `AUTO_INCREMENT` FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='${args[0]}' AND TABLE_NAME='${args[1]}'";
    }
